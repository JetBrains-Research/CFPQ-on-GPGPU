...
int methodOf4Rus_test(int rows, int table_cols_max, int sparsity) {
    ...
    *is_changed_host = NOT_CHANGED;
    ...
    gpuTimer.Start();    
    wrapper_methodOf4Rus_subring(squeezed_matrixA_device, squeezed_matrixB_device, squeezed_matrixC_device, tables, rows, cols, is_changed_host);
    gpuTimer.Stop();
    ...
    delete_matrix_host(is_changed_host);
...
int main() {
    ...
    int max_sparsity =100000;
    int sparsity_step = 100;
    for(int sparsity = 102; sparsity < max_sparsity; sparsity += sparsity_step) {
        for(int size = rows;size < 3000; size += 32) {
            methodOf4Rus_test(size, table_cols_max, sparsity); 
        }  
